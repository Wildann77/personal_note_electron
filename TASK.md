...
19. Testing — Unit Tier
20. Testing — Integration Tier
21. Testing — E2E Tier
...
- [ ] **[P1-T2] Setup TypeScript Strict Mode**
...
- [ ] **[P1-T3] Setup ESLint 9 (Flat Config) + Prettier**
...
- [ ] **[P1-T4] Install & Konfigurasi Native Module Tooling**
...
- [ ] **[P1-T5] Externalize `better-sqlite3` dari Bundle Vite**
...
- [ ] **[P1-T6] Setup Tailwind CSS 4.3.x + CSS Variables Theme**
...
- [ ] **[P1-T7] Setup shadcn/ui (Local Source Ownership) + Radix Primitives**
  ...
  - **File:** `src/renderer/components/ui/` (folder awal, isi diisi di Fase 11)
  - **Kriteria Selesai:** CLI shadcn berhasil generate config (`components.json`) yang mengarah ke struktur folder Architecture §14.
  - **Referensi:** Architecture §2, §14.

- [ ] **[P1-T8] Setup Testing Toolchain (Vitest, Playwright, Testing Library)**
  ...
  - **Kriteria Selesai:** `npm run test:unit` dan `npm run test:e2e` bisa dieksekusi (boleh masih kosong/pass trivially) tanpa error konfigurasi.
  - **Referensi:** Architecture §13, §14.

- [ ] **[P1-T9] Setup Struktur Direktori Clean Architecture Kosong**
...
- [ ] **[P2-T5] Entity `Note` — `Note.ts`**
  ...
  - **File:** `src/main/domain/entities/Note.ts`
  - **Kriteria Selesai:** Entity bisa diuji tanpa mock apa pun.
...
- [ ] **[P2-T8] Domain Errors — `AppError.ts`**
  ...
  - **File:** `src/main/domain/errors/AppError.ts`
  - **Kriteria Selesai:** Setiap `ErrorCode` di §5.1 punya cara jelas untuk dilempar via `AppError`.
  - **Referensi:** Architecture §5.2.

- [ ] **[P2-T9] Domain Service — `NoteContentExtractor.ts`**
  ...
  - **File:** `src/main/domain/services/NoteContentExtractor.ts`
  - **Kriteria Selesai:** Menangani blok kosong (fallback `"Catatan Tanpa Judul"`), berbagai tipe blok, dan membersihkan tag HTML/entity.
  - **Referensi:** Architecture §8.1; PRD US#10, US#11.

- [ ] **[P2-T10] Domain Service — `timeSectioning.ts`**
  ...
  - **File:** `src/shared/utils/timeSectioning.ts`
  - **Kriteria Selesai:** Batas waktu dihitung dari awal hari lokal (bukan 24 jam mundur murni), sesuai contoh implementasi Architecture §8.2.
  - **Referensi:** Architecture §8.2; PRD US#15, US#16.
...
- [ ] **[P3-T1] Koneksi Database & Pemulihan Korupsi — `DatabaseConnection.ts`**
  ...
  - **File:** `src/main/infrastructure/database/DatabaseConnection.ts`
...
- [ ] **[P3-T2] Migration Runner — `migrations.ts`**
  ...
  - **File:** `src/main/infrastructure/database/migrations.ts`
  - **Kriteria Selesai:** `MigrationRunner.run(db)` idempoten — jalan berkali-kali tidak mengulang migrasi yang sama; setiap migrasi dibungkus transaksi.
  - **Referensi:** Architecture §6.2.

- [ ] **[P3-T3] Implementasi Repository SQLite — `SQLiteNoteRepository.ts`**
  ...
  - **File:** `src/main/infrastructure/repositories/SQLiteNoteRepository.ts`
  - **Kriteria Selesai:** Update dengan revisi salah mengembalikan 0 rows affected (bukan overwrite diam-diam); karakter spesial (kutip, simbol) tidak merusak query.
  - **Referensi:** Architecture §3.1 (LSP), §7; PRD US#57, US#59.

- [ ] **[P3-T4] Backup Service — `BackupService.ts`**
  ...
  - **File:** `src/main/infrastructure/backup/BackupService.ts`
...
- [ ] **[P4-T2] Event Hub — `ElectronEventHub.ts`**
  ...
  - **File:** `src/main/infrastructure/events/ElectronEventHub.ts`
...
- [ ] **[P4-T4] Zero-Leak Logger — `logger.ts`**
  ...
  - **File:** `src/main/infrastructure/logger/logger.ts`
...
- [ ] **[P5-T1] `CreateNoteUseCase.ts`**
  ...
  - **File:** `src/main/application/notes/CreateNoteUseCase.ts`
...
- [ ] **[P5-T2] `UpdateNoteUseCase.ts` (OCC Guard)**
  ...
  - **File:** `src/main/application/notes/UpdateNoteUseCase.ts`
  - **Kriteria Selesai:** Konflik revisi terdeteksi dan dikembalikan sebagai error terstruktur, bukan silent overwrite.
  - **Referensi:** Architecture §7.1; PRD US#8, US#9.

- [ ] **[P5-T3] `DeleteNoteUseCase.ts`**
  ...
  - **File:** `src/main/application/notes/DeleteNoteUseCase.ts`
  - **Kriteria Selesai:** Setelah dipanggil berulang cepat (banyak delete beruntun), tidak ada error/unresponsive.
  - **Referensi:** Architecture §17 (Kategori D); PRD US#24, US#27, US#29.

- [ ] **[P5-T4] `GetNotesUseCase.ts`**
  ...
  - **File:** `src/main/application/notes/GetNotesUseCase.ts`
...
- [ ] **[P5-T5] `GetNoteByIdUseCase.ts`**
  ...
  - **File:** `src/main/application/notes/GetNoteByIdUseCase.ts`
...
- [ ] **[P6-T1] Validasi Sender IPC — `validateSender.ts`**
...
- [ ] **[P6-T2] Wrapper Handler Terproteksi — `createHandler.ts`**
  ...
  - **File:** `src/main/ipc/utils/createHandler.ts`
...
- [ ] **[P8-T1] Kebijakan Keamanan Navigasi — `security.ts`**
  ...
  - **Kriteria Selesai:** Klik link di dalam catatan membuka browser sistem, bukan navigasi window Electron.
...
- [ ] **[P9-T1] Notes Store (Runtime, Non-Persist) — `useNotesStore.ts`**
  ...
  - **File:** `src/renderer/stores/useNotesStore.ts`
  - **Kriteria Selesai:** Refresh manual App tidak kehilangan performa karena localStorage besar; setter generik bisa diuji unit terisolasi.
  - **Referensi:** Architecture §17 (PRD Further Note #3); PRD §Testing Decisions poin 3.

- [ ] **[P9-T2] UI Store (Persist Ringan) — `useUIStore.ts`**
  ...
  - **File:** `src/renderer/stores/useUIStore.ts`
  - **Kriteria Selesai:** Isi localStorage app tetap kecil (beberapa KB) berapa pun jumlah catatan pengguna.
...
- [ ] **[P10-T1] Single-Flight Queue — `SingleFlightQueue.ts`**
  ...
  - **File:** `src/renderer/utils/SingleFlightQueue.ts`
  - **Kriteria Selesai:** Mengetik cepat berturut-turut tidak memicu banyak request IPC paralel yang saling menyalip.
...
- [ ] **[P12-T2] `TimeSectionGroup.tsx`**
  ...
  - **Kriteria Selesai:** Urutan grup selalu Hari ini → Kemarin → Sebelumnya; grup kosong tidak tampil sebagai section kosong yang aneh (opsional: sembunyikan section tanpa isi).
...
## Fase 19 — Testing: Unit Tier (Vitest)

- [ ] **[P19-T1] `NoteContentExtractor.test.ts`**
  - **Deskripsi:** Uji ekstraksi title/snippet dari berbagai tipe blok Editor.js (header, paragraph, list, checklist, code, quote), termasuk kasus blok kosong dan konten dengan HTML entity.
  - **File:** `tests/unit/NoteContentExtractor.test.ts`
  - **Kriteria Selesai:** Semua tipe blok yang didukung punya minimal 1 test case, plus edge case blok kosong.
  - **Referensi:** Architecture §13; PRD §Testing Decisions poin 1.

- [ ] **[P19-T2] `timeSectioning.test.ts`**
  - **Deskripsi:** Uji pengelompokan dengan berbagai kombinasi timestamp: tepat hari ini, tepat batas 24 jam lalu, lebih dari 48 jam lalu, dan pastikan urutan dalam tiap grup selalu terbaru-ke-terlama.
  - **File:** `tests/unit/timeSectioning.test.ts`
  - **Kriteria Selesai:** Kasus batas (boundary) tengah malam tercakup, bukan cuma kasus umum.
  - **Referensi:** PRD §Testing Decisions poin 2.

- [ ] **[P19-T3] Uji Setter Generik Store**
  - **Deskripsi:** Uji fungsi setter generik berbasis "nama field + nilai" di `useNotesStore`/`useUIStore`, pastikan tiap jenis update field mengubah bagian state yang benar tanpa memengaruhi bagian lain.
  - **File:** `tests/unit/stores.test.ts`
  - **Kriteria Selesai:** Test mencakup minimal 3 field berbeda dengan asersi isolasi (field lain tidak berubah).
  - **Referensi:** PRD §Testing Decisions poin 3.

---

## Fase 20 — Testing: Integration Tier (Vitest + SQLite `:memory:`)

- [ ] **[P20-T1] `NoteRepository.test.ts`**
  - **Deskripsi:** Uji terhadap SQLite in-memory: simpan catatan baru → muncul di `getAll`; simpan ulang id sama → update (bukan duplikat); ambil by id sesuai; hapus → hilang dari `getAll`; input karakter spesial (kutip, simbol, emoji) tidak menyebabkan kegagalan.
  - **File:** `tests/integration/NoteRepository.test.ts`
  - **Kriteria Selesai:** Seluruh skenario yang disebutkan eksplisit di PRD §Testing Decisions poin 1 tercakup.
  - **Referensi:** Architecture §3.1 (LSP — repository bisa diganti in-memory tanpa ubah use case); PRD §Testing Decisions poin 1.

- [ ] **[P20-T2] `UpdateNoteOCC.test.ts`**
  - **Deskripsi:** Simulasikan dua "window" melakukan update dengan `expectedRevision` yang sama secara berurutan — update kedua harus gagal dengan `CONCURRENCY_ERROR`, bukan menimpa diam-diam.
  - **File:** `tests/integration/UpdateNoteOCC.test.ts`
  - **Kriteria Selesai:** Skenario race condition konkret tervalidasi otomatis, bukan hanya manual.
  - **Referensi:** Architecture §7.1.

- [ ] **[P20-T3] `MigrationRunner.test.ts`**
  - **Deskripsi:** Uji migrasi berjalan sekali dan idempoten (jalan ulang tidak mengulang migrasi yang sama), serta `PRAGMA user_version` bertambah sesuai jumlah migrasi yang diterapkan.
  - **File:** `tests/integration/MigrationRunner.test.ts`
  - **Kriteria Selesai:** Test memverifikasi versi akhir DB sesuai jumlah migrasi terdaftar.
  - **Referensi:** Architecture §6.2.
...
- [ ] **[P21-T1] `multiWindowSync.spec.ts`**
  ...
  - **Kriteria Selesai:** Test gagal jika Further Notes #1 regresi (sinkronisasi lintas window rusak lagi).
...
| 19 | Testing — Unit Tier | 3 |
| 20 | Testing — Integration Tier | 3 |
...
3. Centang `[x]` setelah task selesai dan lulus **Kriteria Selesai**-nya sebelum lanjut ke task berikutnya dalam fase yang sama.
4. Fase 19–21 (testing) idealnya dikerjakan **segera setelah** modul terkait selesai (mis. `NoteContentExtractor.test.ts` bisa langsung setelah P2-T9), bukan ditumpuk semua di akhir — urutan di atas dikelompokkan per tier untuk kejelasan dokumen, bukan berarti harus menunggu semua fitur selesai dulu.
